...
import { getTenantByPhone, getAllTenants } from '../services/tenantService';
...
function getTopItems(orders: any[]): Array<{ name: string; count: number }> {
...
function getOrdersByDay(orders: any[]): Record<string, number> {
...
function exportAsCSV(res: express.Response, data: any): void {
  const csvRows = [
...
async function generateBackup(businessId: string): Promise<{ businessId: string; timestamp: number }> {
